...
@Component({
  selector: 'app-web-page-detail-page',
...
export class WebPageDetailPageComponent implements OnInit, OnDestroy {
  ...
  onSelectedTabChange(tab: MatTabChangeEvent): void {
    switch (tab.index) {
      case 0:
        this.router.navigate(['/', 'secured', 'projects', 'web-pages', this.activatedRoute.snapshot.params['id'], 'config']);
        break;
      case 1:
        this.router.navigate(['/', 'secured', 'projects', 'web-pages', this.activatedRoute.snapshot.params['id'], 'statistics']);
        break;
    }
  }
}
